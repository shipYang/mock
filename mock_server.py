...
def checkpath(path,request_param,request_header,method):
    ...
    if len(l) == 0:
        if request_header!=None:
            for j in range(len(mock_data)):
                if path in mock_data[j] and method in mock_data[j] and request_header in mock_data[j] and mock_data[j][2]=='':
                    return generate_response(mock_data[j][5], mock_data[j][4], mock_data[j][6])

        else:
            for j in range(len(mock_data)):
                if path in mock_data[j] and method in mock_data[j] and mock_data[j][2]=='' and mock_data[j][3]=='':
                    return generate_response(mock_data[j][5], mock_data[j][4], mock_data[j][6])
        ...
        if request_header != None:
            for j in range(len(mock_data)):
                if path in mock_data[j] and (method in mock_data[j]) and (
                    format_reqparam in mock_data[j]) and request_header in mock_data[j]:
                    return generate_response(mock_data[j][5], mock_data[j][4], mock_data[j][6])
        else:
            for j in range(len(mock_data)):
                if path in mock_data[j] and (method in mock_data[j]) and (
                    format_reqparam in mock_data[j]) and mock_data[j][3]=='':
                    return generate_response(mock_data[j][5], mock_data[j][4], mock_data[j][6])
